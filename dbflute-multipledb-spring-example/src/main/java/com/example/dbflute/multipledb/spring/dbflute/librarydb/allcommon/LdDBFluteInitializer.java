...
public class LdDBFluteInitializer {
    ...
    //                                                                          ==========
    /** Log instance. */
    private static final Log _log = LogFactory.getLog(LdDBFluteInitializer.class);
     ...
     */
    public LdDBFluteInitializer(javax.sql.DataSource dataSource) {
        if (dataSource == null) {
    ...
    protected void standBy() {
        if (!LdDBFluteConfig.getInstance().isLocked()) {
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        ...
        if (dataSourceFqcn.startsWith("org.apache.commons.dbcp.")) {
            config.unlock();
            config.setDataSourceHandler(new LdDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }

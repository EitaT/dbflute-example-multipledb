 ...
 */
public class MbDBFluteInitializer {

    // ===================================================================================
    //                                                                          Definition
    //                                                                          ==========
    /** Log instance. */
    private static final Log _log = LogFactory.getLog(MbDBFluteInitializer.class);
    ...
    protected void standBy() {
        if (!MbDBFluteConfig.getInstance().isLocked()) {
            MbDBFluteConfig.getInstance().lock();
        }
    }
    ...
    protected void handleSqlLogRegistry() { // for S2Container
        if (MbDBFluteConfig.getInstance().isUseSqlLogRegistry()) {
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        ...
        if (dataSourceFqcn.startsWith("org.apache.commons.dbcp.")) {
            config.unlock();
            config.setDataSourceHandler(new MbDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }
